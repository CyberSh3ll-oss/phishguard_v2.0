#!/usr/bin/env python3
"""
PhishGuard — Premium GUI Version
"""
...
# ---------- GUI ----------
class PhishGuardPremiumGUI:
    def __init__(self, master):
        self.master = master
        master.title("PhishGuard Premium")
        master.configure(bg="black")
        master.geometry("900x700")
        self.user_type = "free"
        self.ask_premium_key()

        # Title
        self.title_label = tk.Label(master, text="PhishGuard Premium",
                                    font=("Courier New",28,"bold"),
                                    fg="#00ffff" if self.user_type=="premium" else "lime", bg="black")
        self.title_label.pack(pady=15)
        if self.user_type=="premium": self.animate_title_glow()
        ...
        self.scan_button = tk.Button(master, text="Scan URL", command=self.start_scan,
                                     bg="#00ffff" if self.user_type=="premium" else "lime", fg="black")
        ...
        self.result_area = scrolledtext.ScrolledText(master, width=110, height=30,
                                                     bg="black", fg="lime" if self.user_type=="free" else "#00ffff",
                                                     font=("Courier New",10))
        self.result_area.pack(pady=10)

    def ask_premium_key(self):
        key = simpledialog.askstring("Premium Access","Enter premium key (leave blank for Free):",show="*")
        if key and key.strip() == PREMIUM_KEY: self.user_type="premium"
    ...
    def run_scan(self, url):
        ...
        # Checks breakdown
        self.result_area.insert(tk.END,"--- Checks ---\n")
        for k, v in result["checks"].items():
            self.result_area.insert(tk.END,f"{k}: {v}\n")
        self.result_area.insert(tk.END,"\n")

        # Indicators breakdown
        self.result_area.insert(tk.END,"--- Indicators / Heuristic Details ---\n")
        indicators = result["indicators"]
        if self.user_type=="free": indicators = indicators[:3]
        for sev,text in indicators:
            self.result_area.insert(tk.END,f" - {text}\n")

        if self.user_type=="free":
            self.result_area.insert(tk.END,"\nUpgrade to Premium to see full analysis with color-coded heuristics!\n")
        else:
            self.result_area.insert(tk.END,"\nPremium features unlocked: Full heuristic analysis displayed above!\n")

# ---------- Main ----------
if __name__=="__main__":
    root = tk.Tk()
    gui = PhishGuardPremiumGUI(root)
    root.mainloop()
